<?php
	...
	if($_POST['type'] == "register")
	{
		...
		$file_path = $addLogPath;
		$xmlfilename="addLogs.xml";

		/*Log initialization start*/ 
	    $xmlArray = initializeXMLLog("User Registration","website");
	    ...
	    $logMsg = "User Registration process start.";
	    ...
	    $xml_data['parameters']["attributes"]=$post_arr;
	    ...
	    //$xml_data['error code']["attributes"]=$result["errCode"]; 	// it will only print error code

	    $xml_data['insert_query']["attributes"]=$result["query"];
		...
		if(noError($result))
		{
			...
			$returnArr["errCode"] = "-1";
			$returnArr["errMsg"] = "Registration success";
			//$returnArr["demo"] = "returningdemo";
		...
		else
		{
			...
			$returnArr["errCode"] = 5;
			$returnArr["errMsg"] = "Registration Failed";
		}

		$xml_data['response']["data"]="";
    	$xml_data['response']["attributes"]=$returnArr;
		
		$xmlProcessor->writeXML($xmlfilename, $file_path, $xml_data, $xmlArray["activity"]);  //write xml log file
	...
	if($_POST['type'] == "updateData")
	{
		...
		$xmlfilename="editUser.xml";
    	$file_path = $editLogPath;

		$post_arr = array();
		$returnArr = array();

		$xmlArray = initializeXMLLog("Edit User", "website");
    	$xmlProcessor =new xmlProcessor();
	    ...
	    $logMsg = "Edit User Process Start.";
	    $xml_data['step1']["data"] = "1. {$logMsg}";
		...
		$logMsg = "Getting request parameters success.";
        $xml_data["step3"]["data"] = "3. {$logMsg}";
